...
import com.reyco.dasbx.es.support.result.facet.FacetItem;
...
public class VideoServiceImpl implements VideoService {
	...
	public List<String> getSuggestion(String keyword) throws Exception {
		List<String> suggestion = searchTemplate.suggest(Constants.VIDEO_INDEX_NAME, keyword);
		if (suggestion == null) {
	...
	@Override
	public Result<VideoListVO> search(VideoSearchDto videoSearchDto) throws IOException {
		buildSortIfNecessary(videoSearchDto);
		return searchTemplate.search(videoSearchDto, VideoListVO.class);
	}

	@Override
	public Result<VideoListDetailVO> searchBack(VideoListSearchDto videoListSearchDto) throws IOException {
		//
		buildSortIfNecessary(videoListSearchDto);
		//
		Result<VideoListDetailVO> search = searchTemplate.search(videoListSearchDto, VideoListDetailVO.class);
		//
		search.getAggregations().stream().forEach(facetResult -> {
			String name = facetResult.getName();
			if ("category".equals(name)) {
				facetResult.getItems().add(0, new FacetItem("", "全部分类"));
			} else if ("country".equals(name)) {
				facetResult.getItems().add(0, new FacetItem("", "全部地区"));
			} else if ("type".equals(name)) {
				facetResult.getItems().add(0, new FacetItem("", "全部类型"));
			} else if ("year".equals(name)) {
				facetResult.getItems().add(0, new FacetItem("", "全部年份"));
			}
		});
		//
		return search;
	...
	public void add(VideoInsertDto videoInsertDto) throws AuthenticationException {
		...
		VideoProductionInsertPO videoProductionInsertPO = Convert.sourceToTarget(videoProductionInsertDto,
				VideoProductionInsertPO.class);
	...
	public void processPlay(List<PlayEvent> playEvents) {
		Map<Long, Long> playTimeMap = playEvents.stream()
				.collect(Collectors.groupingBy(PlayEvent::getVideoId, Collectors.counting()));

		// 一次性批量更新，而不是分片
		List<VideoPlayPO> videoPlayPOs = playTimeMap.entrySet().stream().map(entry -> {
			...
			while (true) {
				try {
					...
					while (playEvents.size() < 100 && System.currentTimeMillis() - startTime < intervalTime) {
						PlayEvent playEvent = blockingQueue.poll(1000 - (System.currentTimeMillis() - startTime),
	...
	public PlayEventNotify getLeastLoadedPlayEventNotify() {
		...
		// 否则找真正的最小队列
		return playEventNotifys.stream().min(Comparator.comparingInt(p -> p.getBlockingQueue().size()))
				.orElse(selected);
	}
	...
	private void buildSortIfNecessary(VideoSearchDto videoSearchDto) {
		...
		if (sortValue.intValue() == -1) {
			EsSortParam playQuantity = videoSearchDto.getPlayQuantity();
			if (playQuantity == null) {
				playQuantity = new EsSortParam();
				videoSearchDto.setPlayQuantity(playQuantity);
			}
			playQuantity.setSortOrder("desc");

			EsSortParam heatQuantity = videoSearchDto.getHeatQuantity();
			if (heatQuantity == null) {
				heatQuantity = new EsSortParam();
				videoSearchDto.setHeatQuantity(heatQuantity);
			}
			heatQuantity.setSortOrder("desc");
		}
		if (sortValue.intValue() == 1) {
			EsSortParam heatQuantity = videoSearchDto.getHeatQuantity();
			if (heatQuantity == null) {
	...
	private void buildSortIfNecessary(VideoListSearchDto videoSearchDto) {
		Byte sortValue = videoSearchDto.getSort();
		if (sortValue.intValue() == -1) {
			EsSortParam playQuantity = videoSearchDto.getPlayQuantity();
			if (playQuantity == null) {
