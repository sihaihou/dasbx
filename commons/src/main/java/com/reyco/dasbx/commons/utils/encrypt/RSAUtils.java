...
public class RSAUtils {
	...
	public static String encrypt(String source, String publicKey){
		// 1. 防御性检查：避免后续逻辑触发 NullPointerException
	    ...
	    try {
	    	...
	    	// 2. 捕捉所有异常（如 KeyInvalid, NoSuchAlgorithm 等）并记录堆栈，方便线上排查
	        logger.error("RSA加密执行异常: source={}, error={}", source, e.getMessage(), e);
	        return null;
	    }
	}
	/**
	...
	public static String decrypt(String encrypted, String privateKey){
	    ...
	    try {
			...
			// 2. 捕捉所有异常（如 KeyInvalid, NoSuchAlgorithm 等）并记录堆栈，方便线上排查
	        logger.error("RSA解密执行异常: encrypted={}, error={}", encrypted, e.getMessage(), e);
	        return null;
		}
	}
	/**
	 ...
	 * @throws Exception
	 */
	public static Map<String,String> getPublicKeyAndPrivateKey() throws Exception {
		KeyPair keyPair = generateKeyPair();
		PublicKey publicKey = keyPair.getPublic();
		PrivateKey privateKey = keyPair.getPrivate();
		...
		keyMap.put(PUBLIC_KEY_NAME, publicKeyStr);
		keyMap.put(PRIVATE_KEY_NAME, privateKeyStr);
		return keyMap;
	}
